module cudaExt
    ...
    function set_GPUbackend(mem::String="device")
        ...
        if mem == "device"
            backend = Squiggles.metalBackend("GPU", "METAL", mem, Metal.PrivateStorage)

        elseif mem == "unified"
            backend = Squiggles.metalBackend("GPU", "METAL", mem, Metal.SharedStorage)
