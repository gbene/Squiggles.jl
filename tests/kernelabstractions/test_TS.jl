...
function plotCorrelogram(correlograms)
    ...
    points = lift(sl_y.value, sl_z.value) do y, z
        [Point3f(-corr_length*1.1, y, z), Point3f(corr_length*1.1, y, z)]
    end

    text_pos = lift(sl_y.value, sl_z.value) do y, z
        Point3f(-corr_length*1.1, y, z)
    end
...
correlograms = zeros(Float32, n_templates, n_signals, nlags)
...
blocks = (n_templates, n_signals) # Set number of blocks. This is n_templates x n_signals
...
signals_gpu = memcopy(norm_signals)
